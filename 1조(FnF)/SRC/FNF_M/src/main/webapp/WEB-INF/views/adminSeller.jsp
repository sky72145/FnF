	...
	$(document)
			.ready(
					function() {
						...
						$("#newSeller").on("click",".newTr .btnApprove",
										function() {
											var id = $(this).parent().parent()
													.attr("id");
											var cf = confirm("승인하시겠습니까?")
											if (cf) {
												$.ajax({url : "/admin/sellerappr/"+ id,
														method : "PUT",
														contentType : "application/json; charset=utf-8",
														dataType : "json",
														data : JSON.stringify({
														...
														success : function(data) {
															if (data.code) {
																alert("승인이 완료되었습니다.");
																location.href = "/admin/mailappr/"+ id;
															}
														},
														error : function(err) {
															console.log("Error 발생 : "+ err);
						...
						$("#newSeller").on("click",".newTr .btnReject",
										function() {
											var id = $(this).parent().parent()
													.attr("id");
											var cf = confirm("승인을 거절하시겠습니까?")
											if (cf) {
												$.ajax({url : "/admin/sellerrej/"+ id,
														method : "PUT",
														contentType : "application/json; charset=utf-8",
														dataType : "json",
														data : JSON.stringify({
