...
<div class="container">
					...
					<c:forEach items="${cartList}" var="cart">
						...
						<form name="form" id="form" method="get">
							<tr class="text-center">
								...
								<td><a href="/PD/${cart.idx}"><img class="img-rounded" style="width: 100px; height: 100px;" src="img/${cart.filename}"></a></td>
								<td><a href="/PD/${cart.idx}"><strong>${cart.pname}</strong> </a></td>
								<td><fmt:formatNumber value="${cart.price}" pattern="#,###.##" />원</td>
								<td><input type="number" class="text-right" name="product_count" id="product_count"size="2" min="1" max="${cart.stock }" value="${cart.product_count}"></td>
								<td>${product_mil}원</td>
								<td>${fee}원</td>
	...
	<table class="table">
		<td>선택상품을 &nbsp;
			 <input type="submit" class="btn btn-danger btn-sm" id="selectdel" onclick="f_order()" value="삭제하기"/></td>
			 <td class="text-right">
			 <input type="submit" class="btn btn-danger btn-sm" id="modify" onclick="javascript: form.action='/modify/';" value="수정하기">
				</form>
			 <button type="button" id="btnAllDelete" class="btn btn-danger btn-sm text-right">장바구니 비우기</button>
		</td>
	</table>
	<!-- 총액 -->
	<table class="table text-center">
	...
	<div class="text-center">
				<c:choose>
    					<c:when test="${map.count == 0}">
   									 ...
   									 <c:otherwise>
      									<input type="button" class="btn btn-lg btn-primary" id="btnOrder" value="주문" />&nbsp;&nbsp;&nbsp;
    								</c:otherwise>
						</c:choose>
		
		&nbsp;<input type="button" class="btn btn-lg btn-default" value="돌아가기"onclick="history.back()" />
