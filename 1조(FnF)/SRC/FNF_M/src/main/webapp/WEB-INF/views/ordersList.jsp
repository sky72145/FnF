...
//배송중 --> 배송완료
function baesongwanryo(idx){
	if(confirm('변경 처리를 진행하시겠습니까?')){
		$.ajax({
			url : "/baesongwanryo/"+ idx,
			dataType : "json",
			method : "POST",
			success : function(result) {
				//console.log(result.data);
	...
	if (count == 0) {
		...
		for (var i=0 ; i < count; i++) {
			...
			if(count >1 && result[i].idx == result[j].idx && i != j){
	            ...
	            str +="<td>"+writedate+"</td>";
	            if(result[i].status=="입금대기"){
		            str +="<td><a class='text-success' onclick='changeStatus("+result[i].idx+")' href='javascript:void(0);'><strong>결제완료</strong></a></td>";
	            }else if(result[i].status=="배송중"){
		            str +="<td><a class='text-warning' onclick='baesongwanryo("+result[i].idx+")' href='javascript:void(0);'><strong>배송완료</strong></a></td>";
	            }else{
	            	 str +="<td></td>";
	            }
	            str +="<td>";		
