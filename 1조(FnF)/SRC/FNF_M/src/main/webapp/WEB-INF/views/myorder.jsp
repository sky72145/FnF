				...
				<table class="table">
					...
					<tbody>
						<c:forEach items="${order }" var="order" varStatus="status">
							<tr>
								<td><c:out value="${order.idx}" /></td>
								<td><c:out value="${order.del_code}" /></td>
								<td><c:out value="${order.totalprice}" /></td>
								<td><c:out value="${order.status}" /></td>
								<td><c:out value="${order.product_count}" /></td>
								<td><c:out value="${order.pname}" /></td>
								<c:if test="${order.del_code ne null}">
									<td>
										<form id="frn${status.index}" action="/myorder" method="post">
											<input type="hidden" name="idx" id="idx"
												value="<c:out value="${order.idx}" />"> <input
												type="hidden" name="user_id" id="user_id"
												value="<c:out value="${order.user_id}" />">
									...
									<td><button type="button"
											onclick="frmSubmit(${status.index})" id="btnbuy"
											data-oper='modifyorder' class="btn btn-success">구매확정</button>
									</td>
								</c:if>
							<c:choose>
									<c:when test="${order.del_code eq null}">
									<td>
										<form id="frn${status.index}" action="/myorder" method="post">
											<input type="hidden" name="idx" id="idx"
												value="<c:out value="${order.idx}" />"> <input
									...
									<td><button type="button"
											onclick="frmSubmit(${status.index})" id="btnbuy"
											data-oper='modifyorder' class="btn btn-success" disabled>구매확정</button>
									</td>
									</c:when>
								</c:choose>
							</tr>
