				...
				<div class="aa-product-catg-content">
					<div class="aa-product-catg-body">
						<ul class="aa-product-catg">
							...
							<c:forEach items="${productf2 }" var="product">
								<li>
									<figure>
						 	...
						 	<ul class="pagination">
								<c:if test="${pageMaker3.prev}">
									<li class="paginate_button previous"><a
										href='/productf2?category=<c:out value="${pageMaker3.cri.category}" />&pageNum=${pageMaker3.startPage -1}&amount=9'>Previous</a></li>
								</c:if>
								...
								<c:forEach var="num" begin="${pageMaker3.startPage}"
									...
									<li class="paginate_button  ${pageMaker3.cri.pageNum == num ? "active":""} ">
										<a href='/productf2?category=<c:out value="${pageMaker3.cri.category}" />&pageNum=${num}&amount=9'>${num}</a>
									</li>
								</c:forEach>

								<c:if test="${pageMaker3.next}">
									<li class="paginate_button next"><a
										href='/productf2?category=<c:out value="${pageMaker3.cri.category}" />&pageNum=${pageMaker3.endPage +1 }&amount=9'>Next</a></li>	
								</c:if>
							</ul>  
						</nav>
					</div>
