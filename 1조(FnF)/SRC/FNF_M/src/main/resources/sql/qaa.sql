...
create or replace PROCEDURE      QAA_SEARCH
(
...
BEGIN
   OPEN qaa_record FOR
   SELECT B.idx, grp, user_id, title, writedate, readnum, lev, step, contents
   FROM
      (SELECT rownum AS rnum, A.idx, A.grp, A.user_id, A.title, A.writedate, A.readnum, A.lev, A.step, A.contents
   FROM
      (SELECT idx, grp, user_id, title, writedate, readnum, lev, step, contents
...
create or replace PROCEDURE Qaa_SEARCH_TOTAL
(
	s_user_id			    IN		Qaa.user_id%TYPE,
    s_title					IN		Qaa.title%TYPE,
    s_contents				IN		Qaa.contents%TYPE,
