						 	...
						 	<ul class="pagination">
								<c:if test="${pageMaker2.prev}">
									<li class="paginate_button previous"><a
										href='/products2?category2=<c:out value="${pageMaker2.cri2.category2}" />&pageNum=${pageMaker2.startPage -1}&amount=9'>Previous</a></li>
								</c:if>

								<c:forEach var="num" begin="${pageMaker2.startPage}"
									end="${pageMaker2.endPage}">
									<li class="paginate_button  ${pageMaker2.cri2.pageNum == num ? "active":""} ">
										<a href='/products2?category2=<c:out value="${pageMaker2.cri2.category2}" />&pageNum=${num}&amount=9'>${num}</a>
									</li>
								</c:forEach>

								<c:if test="${pageMaker2.next}">
									<li class="paginate_button next"><a
										href='/products2?category2=<c:out value="${pageMaker2.cri2.category2}" />&pageNum=${pageMaker2.endPage +1 }&amount=9'>Next</a></li>	
								</c:if>
							</ul>  
						</nav>
					</div>
